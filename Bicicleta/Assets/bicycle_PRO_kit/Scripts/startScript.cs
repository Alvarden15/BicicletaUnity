...
public class startScript : MonoBehaviour {
	...
	//Deshaste de esto cuando implementes SimpleInput
	...
	void Update () {
		...
		if (Input.GetMouseButtonDown (0)) {
			...
			if (hit.transform.gameObject.name == "dirtBike_Selector") {
				SceneManager.LoadScene("BPK_bicycle_MTB");
				
			}
			if (hit.transform.gameObject.name == "fullSuspBikeSelector") {
				SceneManager.LoadScene("BPK_bicycle_FullSuspension");
				
			}
			if (hit.transform.gameObject.name == "mobDevice_Selector") {
				SceneManager.LoadScene("BPK_bicycle_mobile");
