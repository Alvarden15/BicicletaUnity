// Writen by Boris Chuprin smokerr@mail.ru
using UnityEngine;
using System.Collections;

public class pedalControls : MonoBehaviour
{
    //Aqui es donde se implementan los controles. Haz cambios aqui si deseas que 
    //Funcionen de forma correcta o quieras cambiar de tecla.
    [SerializeField]
    private bicycle_code linkToBike;// making a link to corresponding bike's script

    [SerializeField]
    private biker_logic_mecanim linkToRider;//link to rider's script

    public GameObject pedalLeft;
    public GameObject pedalRight;

    [SerializeField]
    private GameObject ctrlHub;// gameobject with script control variables 
    private controlHub outsideControls;// making a link to corresponding bike's script

    private float energy = 0;//energy of pedaling to release after acceleration off

    //we need it to move CoM left and right to immitate rotation of pedals
    public Transform CoM; //CoM object

    //to move pelvis of rider
    public Transform veloMan;

    //BIKE for stunts
    public Transform stuntBike;

    Rigidbody cuerpoStunt;

    //special temp status "on stunt" to prevent fall after maximus angle exceed
    public bool stuntIsOn = false;

    //tmp "true" during "in stunt" 
    private bool inStunt = false;
    private void Start()
    {

        //ctrlHub = GameObject.Find("gameScenario");
        //link to GameObject with script "controlHub"
        outsideControls = ctrlHub.GetComponent<controlHub>();
        cuerpoStunt = stuntBike.GetComponent<Rigidbody>();
        
        //to connect c# mobile control script to this one

        //linkToBike = GameObject.Find("rigid_bike").GetComponent<bicycle_code>();
        //linkToRider = GameObject.Find("char_anim").GetComponent<biker_logic_mecanim>();

    }

    private void Update()
    {
     //Pressing "Space" starts bunnyhop stunt
        if (Input.GetKeyDown(KeyCode.Space) || Input.GetButtonDown("Jump"))
        {
            //Timing.RunCoroutine(StuntBunnyHope());
            StartCoroutine(StuntBunnyHope());
        }

        if (Input.GetKeyDown(KeyCode.N))
        {
            StartCoroutine(StuntBackFlip360());
        }

        if (Input.GetKeyDown(KeyCode.M))
        {
            StartCoroutine(StuntTurnLeft180());
        }

        if (Input.GetKeyDown(KeyCode.B))
        {
            StartCoroutine(StuntBunnyShiftRight());
        }

        if (Input.GetKeyDown(KeyCode.Slash))
        {
            StartCoroutine(StuntHoldForOneSecond());
        }
        //"2" for manual
        if (Input.GetKeyDown(KeyCode.Alpha2))
        {
            StuntManual();
        }

    }

    private void FixedUpdate()
    {
        //pedals rotation part
        if (outsideControls.Vertical > 0)
        {
            transform.rotation *= Quaternion.Euler(linkToBike.bikeSpeed / 4, 0, 0);
            pedalRight.transform.rotation *= Quaternion.Euler(-linkToBike.bikeSpeed / -4, 0, 0);
            pedalLeft.transform.rotation *= Quaternion.Euler(-linkToBike.bikeSpeed / 4, 0, 0);
            if (energy < 10)
            {
                energy += 0.01f;
            }

            if (Mathf.Abs(CoM.transform.localPosition.x) < 0.1f)
            {
                var tmpRidPlvs01 = veloMan.transform.localEulerAngles;
                tmpRidPlvs01.z = CoM.transform.localPosition.x * 200;
                veloMan.transform.localEulerAngles = tmpRidPlvs01;
                //(sometimes looks strange on bicycles with high seat. So, you might just disable it when needed)
            }
            var tmpCoM01 = CoM.transform.localPosition;
            tmpCoM01.x = -0.02f + (Mathf.Abs(transform.localRotation.x) / 25);//leaning bicycle when pedaling
            CoM.transform.localPosition = tmpCoM01;

        }
        else EnergyWaste();//need to move pedals some time after stop acceleration

        //movement body of rider's pelvis when cornering(sometimes looks strange on bicycles with high seat. So, you might just disable it when needed)
        var tmpRidPlvs02 = veloMan.transform.localPosition;
        tmpRidPlvs02.x = outsideControls.Horizontal / 10;
        veloMan.transform.localPosition = tmpRidPlvs02;



    }

    //function when player stop accelerating and rider still slowly rotating pedals
    private void EnergyWaste()
    {
        if (!(energy > 0)) return;

        var tmpEnergy = 10 - energy;
        transform.rotation *= Quaternion.Euler((linkToBike.bikeSpeed - tmpEnergy) / 4, 0, 0);
        pedalRight.transform.rotation *= Quaternion.Euler(-(linkToBike.bikeSpeed - tmpEnergy) / -4, 0, 0);
        pedalLeft.transform.rotation *= Quaternion.Euler(-(linkToBike.bikeSpeed - tmpEnergy) / 4, 0, 0);
        energy -= 0.1f;
    }

    //trick to do not crash for one second. You need that for riding ramps
    private IEnumerator StuntHoldForOneSecond()
    {
        stuntIsOn = true;
        yield return new WaitForSeconds(0.5f);//1 second seems too long :) now it's half of second 0.5ff. Make 1 for actually 1 second
        if (!inStunt)
        {
            stuntIsOn = false;
        }
    }
    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    // applying physical forces to immitate stunts///////////////////////////////////////////////////////////////////////////////////////
    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    //void StuntBunnyHope (){
    private IEnumerator StuntBunnyHope()
    {
        linkToRider.PlayA("bannyhope");//animation is optional. You may delete this string with no bad aftermath
        cuerpoStunt.AddForce(Vector3.up * 40000);//push bike up

        yield return new WaitForSeconds(0.1f);//a little pause between applying force
        cuerpoStunt.AddTorque(transform.right * -14000);//pull front wheel(turn bike around CoM)

        yield return new WaitForSeconds(0.2f);//a little pause between applying force
        cuerpoStunt.AddTorque(transform.right * 20000);//push front down and pull rear up

    }
    private void StuntManual()
    {
        linkToRider.PlayA("manual");
    }

    //here is stunts
    private IEnumerator StuntBackFlip360()
    {
        linkToRider.PlayA("backflip360");
        stuntIsOn = true;
        inStunt = true;
        //CoM.transform.localPosition.y = 0;
        var tmpCoM01 = CoM.transform.localPosition;
        tmpCoM01.y = 0;
        CoM.transform.localPosition = tmpCoM01;

        cuerpoStunt.AddTorque(transform.right * -2500000);
        
        yield return new WaitForSeconds(0.1f);
        cuerpoStunt.AddTorque(transform.right * -2500000);
        
        yield return new WaitForSeconds(0.1f);

        cuerpoStunt.AddTorque(transform.right * -2500000);
        
        yield return new WaitForSeconds(0.1f);
        cuerpoStunt.AddTorque(transform.right * -2500000);
        
        yield return new WaitForSeconds(0.1f);

        cuerpoStunt.AddTorque(transform.right * -2500000);
        
        yield return new WaitForSeconds(0.1f);

        cuerpoStunt.AddTorque(transform.right * -2500000);
        
        yield return new WaitForSeconds(0.1f);

        cuerpoStunt.AddTorque(transform.right * -2500000);
        
        yield return new WaitForSeconds(0.1f);

        cuerpoStunt.AddTorque(transform.right * -2500000);
        
        yield return new WaitForSeconds(0.1f);

        cuerpoStunt.AddTorque(transform.right * -2500000);

        yield return new WaitForSeconds(0.1f);

        cuerpoStunt.AddTorque(transform.right * -2500000);
        
        yield return new WaitForSeconds(0.1f);

        cuerpoStunt.AddTorque(transform.right * -2500000);
        yield return new WaitForSeconds(0.7f);
        inStunt = false;
        stuntIsOn = false;
    }

    private IEnumerator StuntTurnLeft180()
    {
        linkToRider.PlayA("rightflip180");
        stuntIsOn = true;
        inStunt = true;
        var tmpCoM02 = CoM.transform.localPosition;
        tmpCoM02.y = 0;
        CoM.transform.localPosition = tmpCoM02;
        cuerpoStunt.AddRelativeTorque(Vector3.up * 10000);
        //cuerpoStunt.AddRelativeTorque(Vector3.up * 20000);

        yield return new WaitForSeconds(0.1f);

        cuerpoStunt.AddRelativeTorque(Vector3.up * 10000);
        //cuerpoStunt.AddRelativeTorque(Vector3.up * 20000);

        yield return new WaitForSeconds(0.1f);

        cuerpoStunt.AddRelativeTorque(Vector3.up * 10000);
        //cuerpoStunt.AddRelativeTorque(Vector3.up * 20000);
        yield return new WaitForSeconds(0.1f);

        cuerpoStunt.AddRelativeTorque(Vector3.up * 10000);
        //cuerpoStunt.AddRelativeTorque(Vector3.up * 20000);
        yield return new WaitForSeconds(0.1f);

        cuerpoStunt.AddRelativeTorque(Vector3.up * 10000);
        //cuerpoStunt.AddRelativeTorque(Vector3.up * 20000);
        yield return new WaitForSeconds(0.1f);

        cuerpoStunt.AddRelativeTorque(Vector3.up * 10000);
        //cuerpoStunt.AddRelativeTorque(Vector3.up * 5000);
        yield return new WaitForSeconds(0.7f);
        inStunt = false;
        stuntIsOn = false;
    }

    private IEnumerator StuntBunnyShiftRight()
    {
        linkToRider.PlayA("bannyhope");

        cuerpoStunt.AddForce(Vector3.up * 45000);//push bike up

        yield return new WaitForSeconds(0.1f);

        cuerpoStunt.AddRelativeTorque(Vector3.right * -4000);//pull front wheel(turn bike around CoM)
        yield return new WaitForSeconds(0.1f);

        cuerpoStunt.AddRelativeTorque(Vector3.up * 1000);//turn bike right
        yield return new WaitForSeconds(0.1f);

        cuerpoStunt.AddRelativeForce(Vector3.right * 24000);//push bike right
        yield return new WaitForSeconds(0.2f);

        cuerpoStunt.AddRelativeTorque(Vector3.up * -3000);//turn bike left


    }
}