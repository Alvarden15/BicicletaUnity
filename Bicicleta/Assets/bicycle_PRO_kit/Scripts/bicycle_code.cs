...
public class bicycle_code : MonoBehaviour
{
    ...
    private bool forgeBlocked = true; // variable to lock front forge for front braking
                                      ...
                                      //If your vehicle is not 4 wheels or mass is not 1600 but 400 - you are in trouble.
    ...
    private float baseDistance; // need to know distance between wheels - base. It's for wheelie compensate(do not want wheelie for long bikes)
    ...
    /*EL OnGUI no será necesario para cuando contruyamos la aplicacion propiamente dicho*/
    ...
    void OnGUI()
    {
        ...
        bikeSpeed es para revisar la velocidad de la bicicleta. De ahi es donde se puede obtener más informacion
        para ver como alterar esa variable
    ...
    */
    ...
    void Start()
    {
        ...
        cuerpoBicicleta = GetComponent<Rigidbody>();

        Vector3 setInitialTensor = cuerpoBicicleta.inertiaTensor;//this string is necessary for Unity 5.3f with new PhysX feature when Tensor decoupled from center of mass
        cuerpoBicicleta.centerOfMass = new Vector3(CoM.localPosition.x, CoM.localPosition.y, CoM.localPosition.z);// now Center of Mass(CoM) is alligned to GameObject "CoM"
        cuerpoBicicleta.inertiaTensor = setInitialTensor;////this string is necessary for Unity 5.3f with new PhysX feature when Tensor decoupled from center of mass
        ...
        */
        ...
        cuerpoBicicleta.interpolation = RigidbodyInterpolation.Interpolate;
    ...
    void FixedUpdate()
    {
        ...
        if (!crashed)
        {
            cuerpoBicicleta.drag = cuerpoBicicleta.velocity.magnitude / 210 * airRes; // when 250 bike can easy beat 200km/h // ~55 m/s
            cuerpoBicicleta.angularDrag = 7 + cuerpoBicicleta.velocity.magnitude / 20;
        }

        //determinate the bike speed in km/h
        bikeSpeed = Mathf.Round((cuerpoBicicleta.velocity.magnitude * 3.6f) * 10) * 0.1f; //from m/s to km/h
        ...
        {//case with acceleration from 0.0f to 0.9f throttle
            coll_frontWheel.brakeTorque = 0;//we need that to fix strange unity bug when bike stuck if you press "accelerate" just after "brake".
            ...
            cuerpoBicicleta.centerOfMass = new Vector3(CoM.localPosition.x, CoM.localPosition.y, CoM.localPosition.z);
        }
        //case for reverse
        if (!crashed && outsideControls.Vertical > 0 && isReverseOn)
        {
        ...
        if (!crashed && outsideControls.Vertical > 0.9f && !isReverseOn)// acceleration >0.9f throttle for wheelie	
        {
            ...
            //meshRearWheel.GetComponent<Renderer>().material.color = Color.green;
            cuerpoBicicleta.angularDrag = 20;//for wheelie stability
            ...
            // need to suppress wheelie when leaning because it's always fall and it't not fun at all
            float angleLeanCompensate = 0.0f;
            if (transform.localEulerAngles.z < 70)
            {
                angleLeanCompensate = transform.localEulerAngles.z / 30;
                if (angleLeanCompensate > 0.5f)
                {
                    angleLeanCompensate = 0.5f;
                }
            }
            if (transform.localEulerAngles.z > 290)
            {
                angleLeanCompensate = (360 - transform.localEulerAngles.z) / 30;
                if (angleLeanCompensate > 0.5f)
                {
                    angleLeanCompensate = 0.5f;
                }
        ...
        if (!crashed && outsideControls.Vertical < 0 && !isFrontWheelInAir)
        {
            ...
            else if (bikeSpeed <= 1 && !crashed && transform.localEulerAngles.z < 45 || bikeSpeed <= 1 && !crashed && transform.localEulerAngles.z > 315)
            {
                if (transform.localEulerAngles.x < 5 || transform.localEulerAngles.x > 355)
                {
            ...
            if (CoM.localPosition.z >= 0.2f + (cuerpoBicicleta.mass / 500))
            {
                CoM.localPosition = new Vector3(CoM.localPosition.x, 0.2f + (cuerpoBicicleta.mass / 500), CoM.localPosition.z);
            }
        ...
        if (!crashed && outsideControls.rearBrakeOn)
        {
            ...
            if (transform.localEulerAngles.x > 180 && transform.localEulerAngles.x < 350)
            {
        ...
        */
        ...
        //Puede o no que lo necesitemos. SI el usuario necesita que se reinicie todo el nivel, lo quitamos.
        //Si no, entonces lo dejamos. Pero sugiriria que en su lugar lo refactoricemos en un metodo aparte.
        if (outsideControls.restartBike)
        {
            ...
            cuerpoBicicleta.velocity = Vector3.zero;
            cuerpoBicicleta.angularVelocity = Vector3.zero;
        ...
        if ((transform.localEulerAngles.z >= crashAngle01 && transform.localEulerAngles.z <= crashAngle02) && !linkToStunt.stuntIsOn || (transform.localEulerAngles.x >= crashAngle03 && transform.localEulerAngles.x <= crashAngle04 && !linkToStunt.stuntIsOn))
        {
            cuerpoBicicleta.drag = 0.1f; // when 250 bike can easy beat 200km/h // ~55 m/s
            cuerpoBicicleta.angularDrag = 0.01f;
            crashed = true;
    ...
    private void ApplyLocalPositionToVisuals(WheelCollider collider)
    {
        ...
        // Es posible que mejor utilices Tags en lugar de nombres
        if (!rearPend)
        {//case where MTB have no rear suspension
            if(!collider.CompareTag("LlantaTrasera"))
            //if (collider.gameObject.name != "coll_rear_wheel")
            {
                if (Physics.Raycast(wheelCCenter, -collider.transform.up, out hit, collider.suspensionDistance + collider.radius))
                {
                    ...
                    if(collider.CompareTag("LlantaFrontal")) isFrontWheelInAir = false;
                ...
                else
                {
                    ...
                    if(collider.CompareTag("LlantaFrontal")) isFrontWheelInAir = true;
